<?php
	...
	if($_SERVER['REQUEST_METHOD']=="POST")
	{
		if(empty($_REQUEST['search1']))
		{
			...
			header("Location:../View/AllVehicleInfo.php");
		}
		else
		{
			...
			$sql = "DELETE FROM vehicle WHERE vnumber = ?";
			...
			if($stmt->execute())
			{
				...
				if(is_numeric($del_value))
				{
					...
					$message = "Your Vehicle Information Deleted by an Admin ($first $last) from VMS. ";
				...
				else
				{
					...
					$body = "Your Vehicle Information Deleted by an Admin ($first $last) from VMS.";
